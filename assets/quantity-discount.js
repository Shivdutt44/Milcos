...
/* provided dependency */ var QuantityDiscount_MinimogSettings = __webpack_require__(4558)["MinimogSettings"];
function QuantityUpsell(_ref) {
  ...
  const variant = product?.variants?.find(v => v.id === Number(selectedVariant)) || product?.variants[0];
...
/* provided dependency */ var quantity_discount_MinimogSettings = __webpack_require__(4558)["MinimogSettings"];
/* provided dependency */ var quantity_discount_MinimogTheme = __webpack_require__(4558)["MinimogTheme"];
  ...
  constructor(settings) {
    ...
    (0,defineProperty/* default */.Z)(this, "listenEvents", () => {
      ...
      MinimogEvents.subscribe(`${this.productId}__VARIANT_CHANGE`, (variant, prodInstance) => {
        if (!prodInstance.view) {
          const isMainProduct = prodInstance.closest('.product-form')?.classList.contains('main-product');
    ...
    (0,defineProperty/* default */.Z)(this, "addItemsToCart", async button => {
      ...
      const res = await addToCart({
        ...
        if (!quantity_discount_MinimogSettings.use_ajax_atc) {
          window.location.href = quantity_discount_MinimogSettings.routes?.cart || '/cart';
        }
      });

      if (res?.status === 422) {
        return quantity_discount_MinimogTheme.Notification.show({
          target: this?.domNodes?.formError,
          method: 'appendChild',
          type: 'warning',
          message: res?.description || "Unprocessable Entity"
        });
      }
    ...
    (0,defineProperty/* default */.Z)(this, "getQuantityToAdd", quantity => {
      const {
        items = []
      } = quantity_discount_MinimogTheme?.Cart?.cart || {};
      let totalAdded = 0;
      items.forEach(_ref => {
        let {
          product_id,
    ...
    (0,defineProperty/* default */.Z)(this, "getQuantityAdded", cart => {
      const items = cart ? cart.items : quantity_discount_MinimogTheme?.Cart?.cart?.items;
      let totalAdded = 0;
    ...
    (0,defineProperty/* default */.Z)(this, "updateButtonStatus", cart => {
      ...
      this.settings?.discounts?.forEach((_ref3, index) => {
        let {
          min_qty
        } = _ref3;
    ...
    (0,defineProperty/* default */.Z)(this, "toggleSpinner", (target, status) => {
      ...
      this.domNodes.productList?.classList[method](this.classes.adding);
    ...
    this.productId = Number(this.productForm?.dataset?.productId);
    this.init();
  }
