  ...
  initFlashSale: async function () {
    ...
    if (res.ok && res.payload && res.payload.length) {
      ...
      this.domNodes.products.forEach(card => {
        ...
        const content = card.querySelector('.sf__pcard-content');
        const soldNumber = card.dataset?.soldNumber; // const component = <SaleProgress settings={res.payload[0]} productId={card.dataset.productId} soldNumber={soldNumber} />

        const component = saleProgress(res.payload[0], card.dataset.productId, soldNumber); // const dom = new DOMParser().parseFromString(component, 'text/html')
