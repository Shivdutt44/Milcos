...
/* provided dependency */ var lucky_wheel_MinimogTheme = __webpack_require__(4558)["MinimogTheme"];
/* provided dependency */ var lucky_wheel_MinimogSettings = __webpack_require__(4558)["MinimogSettings"];
...
class LuckyWheel {
  constructor(settings) {
    ...
    (0,defineProperty/* default */.Z)(this, "handleAddEvents", () => {
      ...
      playBtn?.addEventListener('click', this.handleSubmit); // Close screen
    ...
    (0,defineProperty/* default */.Z)(this, "setupAnimation", () => {
      const duration = this.settings?.trigger === 'exit_intent' ? 0.6 : 1;
    ...
    (0,defineProperty/* default */.Z)(this, "stopAt", () => {
      ...
      if (!prizes?.length) return DEFAULT_TURNS;
    ...
    (0,defineProperty/* default */.Z)(this, "handleOpen", () => {
      ...
      if (cookieData && cookieData?.opened) {
        if (teaser_activate && cookieData?.converted !== true && cookieData?.dismissed !== true && teaser_when === 'after') {
          this.showTeaser();
        }
    ...
    (0,defineProperty/* default */.Z)(this, "handleShowPrize", () => {
      ...
      const prize = this.settings?.prizes[this.lastIndex];
      ...
      if (prize) {
        ...
        if (hasCode) {
          ...
          this.domNodes.discountLabel.innerHTML = this.settings?.lose_label;
        }
      }
    ...
    (0,defineProperty/* default */.Z)(this, "validateField", emailVal => {
      ...
      if (this.settings.show_agree_checkbox && !agree_terms.checked) {
        agreement?.classList?.add?.('shake');
        checked = false;
      } else {
        agreement?.classList?.remove?.('shake');
        checked = true;
      }
    ...
    if (this.settings?.display_on === 'home_only' && lucky_wheel_MinimogSettings.template === 'index') {
      this.init();
    }

    if (this.settings?.display_on === 'all_pages') {
      this.init();
    }
  }
