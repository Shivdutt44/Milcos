...
register('product-tabs', {
  ...
  initTabs: function () {
    this.tabs = new product_tabs_MinimogTheme.Tabs(this?.container, target => {
      const tabId = target.getAttribute('href');
      const slider = this.container.querySelector(tabId + ' .swiper-container');
      const controlsContainer = this.container.querySelector(tabId + ' .sf-slider__controls'); // trigger update slider

      slider && slider.swiper.update();
      const firstItem = slider?.querySelector('.sf-image');
  ...
  initMobileSelect: function () {
    ...
    this.select.addEventListener('change', () => {
      ...
      const controlsContainer = this.tabs?.currentTab?.querySelector('.sf-slider__controls');
      slider && slider.swiper.update();
      const firstItem = slider?.querySelector('.sf-image');
