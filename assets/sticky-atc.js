...
if (!customElements.get("sticky-atc")) {
  customElements.define("sticky-atc", class StickyAtc extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this.mainATCButton = this.mainProduct?.querySelector(this.selectors.atc);
      this.mainProductDynamic = this.mainProduct?.querySelector(this.selectors.buyNowBtn);
      this.stickyATCBar = document.querySelector(".prod__sticky-atc");
      this.productId = this.dataset.productId;
      this.domNodes = queryDomNodes(this.selectors, this.stickyATCBar);
      this.init();
    }
    ...
    init() {
      ...
      prodTitle?.addEventListener("click", window.MinimogLibs.scrollToTop);
      mainImage?.addEventListener("click", window.MinimogLibs.scrollToTop);

      if (this.hasCustomFields) {
        atc.addEventListener("click", e => {
          e.preventDefault();
          e.stopPropagation();
          window.MinimogLibs.scrollToTop(() => this.mainATCButton.click());
        });
        ...
        if (buyNowBtn) {
          buyNowBtn.addEventListener("click", e => {
            ...
            if (missing.length > 0) {
              e.preventDefault();
              e.stopPropagation();
              window.MinimogLibs.scrollToTop(() => this.mainProductDynamic?.click());
            }
          }, true);
        }
      }
